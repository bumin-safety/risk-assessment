...
🔗 **배포 위치:** https://bumin-safety.github.io/risk-assessment/

## 📦 파일 구성

| 파일 | 용도 | 위치 |
|---|---|---|
| `index.html` | 메인 웹앱 (단일 파일) | `risk-assessment` 저장소 루트 |
| `Code.gs` | Google Apps Script (이메일 PDF 첨부 발송) | Apps Script 프로젝트에 붙여넣기 |
| `README.md` | 본 안내서 | `risk-assessment` 저장소 루트 |

## 🚀 배포 순서

### 1️⃣ GitHub 저장소 생성 및 파일 업로드 (완료된 단계)
```
- GitHub 계정/조직: bumin-safety
- 저장소명: risk-assessment
- 배포 URL: https://bumin-safety.github.io/risk-assessment/
- Settings → Pages → main / root 활성화
```
...
## 🗂 향후 확장 (bumin-safety 계정 내)

`bumin-safety` 조직 아래에 도구별로 저장소를 추가하면 동일한 패턴으로 운영 가능:

```
bumin-safety (조직)
├── risk-assessment   ← 본 도구 (수시 위험성평가)
├── patrol            ← 작업장순회점검 (예정)
├── msds              ← MSDS 관리 (예정)
└── ...
```

각각 `https://bumin-safety.github.io/{repo-name}/` 으로 접속.

## ✅ 배포 체크리스트

- [x] `bumin-safety/risk-assessment` 저장소 생성 (Public)
- [x] `index.html`, `README.md`, `Code.gs` 업로드
- [ ] GitHub Pages 활성화 (Settings → Pages → main branch)
- [ ] https://bumin-safety.github.io/risk-assessment/ 접속 확인
...
- 권장 대안:
  ...
  2. Private 저장소 + 다른 호스팅 검토 (Cloudflare Pages 등)
  3. GAS를 프록시로 활용해 Claude API 호출도 GAS 경유 (키를 GAS에 보관)
